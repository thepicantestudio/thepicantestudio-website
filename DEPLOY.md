...
- `_headers` sets HSTS, a Content-Security-Policy, X-Frame-Options, Permissions-Policy, Referrer-Policy and nosniff on every static response. The CSP allows only same-origin scripts plus the sha256 hash of the one inline script in index.html (the intro prelaunch snippet). If that snippet ever changes, recompute the hash: `python -c "import re,hashlib,base64;s=open('index.html',encoding='utf-8').read();m=re.search(r'<script>(.*?)</script>',s,re.S);print(base64.b64encode(hashlib.sha256(m.group(1).encode()).digest()).decode())"` and paste it into `_headers`, or the intro silently stops running.
...
- Responses generated by `src/worker.js` (the lead API and video byte ranges) carry the same headers from `SECURITY_HEADERS` in the script, because `_headers` does not apply to Worker-generated responses.
- `POST /api/lead` requires `application/json`, caps the body at 4 KB, checks the Origin header, drops honeypot submissions, and is rate limited to five submissions per minute per IP through the `LEAD_RL` binding in `wrangler.jsonc` (Workers rate limiting, needs Wrangler 4.36 or newer; the form still works if the binding is absent).
- The Worker redirects plain HTTP to HTTPS on the paths it handles. The site-wide redirect is the Cloudflare dashboard setting SSL/TLS > Edge Certificates > Always Use HTTPS, which must be on.
