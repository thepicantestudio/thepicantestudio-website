...
- `POST /api/lead` requires `application/json`, caps the body at 4 KB, checks the Origin header, drops honeypot submissions, and is rate limited to five submissions per minute per IP through the `LEAD_RL` binding in `wrangler.jsonc` (declared under `unsafe.bindings`, the form still works if the binding is absent). Check it after a deploy: a dozen rapid honeypot posts from one machine should start returning 429 after the fifth.
